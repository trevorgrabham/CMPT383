...
mod queue_tests {
    ...
    const DELAY: time::Duration = Duration::from_millis(50);
    ...
    fn concurrently() {
        ...
        assert!(time_taken as f64 <= (target_time as f64) * 1.3,
                format!("Queue appears to not be running tasks concurrently: n_workers tasks should be happening in parallel. Took {} ms, expected {} ms,", time_taken, target_time)
        );

        assert!(time_taken as f64 > (target_time as f64) * 0.9,
                format!("Queue appears to be running too concurrently: it should only start n_workers concurrent tasks. Took {} ms, expected {} ms,", time_taken, target_time)
        );
    }
    ...
    fn stop() {
        ...
        let final_n_run = sync::Arc::try_unwrap(n_run).unwrap().load(Ordering::SeqCst);
        assert!(
            final_n_run <= n_threads * 3,
            format!("too many tasks executed: {}", final_n_run)
        );
        assert!(
            final_n_run >= n_threads * 2,
            format!("not enough tasks executed: {}", final_n_run)
        );
    }
